import { ArrowLeft, Flag, Zap, Award, Car, User, Trophy, Radio } from 'lucide-react';
...
export default function MaxLegacyPage() {
  ...
  const timelineEvents = [
    ...
    { year: "2003-2013", icon: <Car size={20}/>, title: "Karting Miracles", desc: "From Minimax to KZ2, Max dominated karting globally, winning nearly every championship he entered with blistering speed and relentless racecraft." },
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-gray-200 selection:bg-red-600 selection:text-white pb-20 font-sans">
      ...
      <div className="container mx-auto px-6 py-12 max-w-5xl space-y-16">
        ...
        {/* 英雄區塊 (Jason 的感受故事) */}
        <section className="bg-slate-900 p-8 md:p-12 rounded-3xl border border-blue-900/50 relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
          <div className="absolute inset-0 z-10 bg-gradient-to-r from-slate-950 to-transparent pointer-events-none"></div>
          <div className="relative z-20 flex-1 space-y-5 text-center md:text-left">
            <h1 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-wide leading-tight">Growing Up <br className="hidden md:block"/>With <span className="text-red-600">Max</span></h1>
            <div className="w-16 h-1.5 bg-red-600 rounded-full mx-auto md:mx-0"></div>
            <p className="text-gray-300 leading-relaxed text-base md:text-lg max-w-xl">
        ...
        <section className="space-y-10">
          <div className="flex items-center gap-3 pl-4 border-l-4 border-yellow-400">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-white uppercase tracking-wider">Milestones & History</h2>
          </div>
          ...
          <div className="relative space-y-8 before:absolute before:inset-y-0 before:left-4 before:w-0.5 before:bg-slate-800 before:z-0 pl-12">
            {timelineEvents.map((event, i) => (
              <div key={i} className="relative z-10 group flex flex-col md:flex-row gap-4 bg-slate-900/60 p-5 md:p-6 rounded-2xl border border-slate-800 hover:border-yellow-600/50 transition">
                {/* 年份圖示 */}
                ...
                <div className="md:w-32 shrink-0 flex items-start">
                  <span className="text-2xl md:text-3xl font-black text-white italic tracking-tighter opacity-70">{event.year}</span>
                </div>
                <div className="flex-1 space-y-1">
                  <h3 className="text-lg md:text-xl font-bold text-yellow-400 uppercase tracking-wide">{event.title}</h3>
